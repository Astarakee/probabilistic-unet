...
class LinearBatchGenerator(SlimDataLoaderBase):
        ...
        try:
            ...
            for subject in sorted(data.keys()):
                ...
                if current_timesteps < time_size:
                    continue
                for i in range(current_timesteps - time_size + 1):
                    sets.append((subject, slice(i, i + time_size)))
            _possible_sets = sets
            return sets
    ...
    def __len__(self):
        ...
        return int(np.ceil(len(_possible_sets) / float(batch_size)))
    ...
    def make_batch(self, idx):
        ...
        while len(batch_data) < batch_size:
            ...
            idx = 0
            subject, slc = sets[data_order[idx]]
            current_data = data[subject][slc][:, channels, ...]

            current_data = np.concatenate(current_data)
            current_seg = data[subject][slc][:, 5:, ...]
